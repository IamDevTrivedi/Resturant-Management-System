...
export const Navbar = () => {
  ...
  const [mounted, setMounted] = useState(false); // 👈 important fix for hydration
  ...
  // Prevent hydration mismatch (wait for client)
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  return (
    <header>
        ...
        >
          <div className="relative flex flex-wrap items-center justify-between gap-6 py-3 lg:gap-0 lg:py-4">
            ...
            <div className="bg-background in-data-[state=active]:block lg:in-data-[state=active]:flex mb-6 hidden w-full flex-wrap items-center justify-end space-y-8 rounded-3xl border p-6 shadow-2xl shadow-zinc-300/20 md:flex-nowrap lg:m-0 lg:flex lg:w-fit lg:gap-6 lg:space-y-0 lg:border-transparent lg:bg-transparent lg:p-0 lg:shadow-none dark:shadow-none dark:lg:bg-transparent">
              ...
              {/* Auth Buttons — rendered only after client is mounted */}
              {mounted && (
                <div className="flex w-full flex-col space-y-3 sm:flex-row sm:gap-3 sm:space-y-0 md:w-fit">
                  {!isAuthenticated() ? (
                    <>
                      ...
                      >
                        <UserPlus2Icon className="mr-1 h-4 w-4" />
                        Create Account
                      </Button>
                    </>
                  ) : (
      ...
      <div className="mb-9 text-transparent">THIS IS A NAVBAR</div>
    </header>
  );
};
